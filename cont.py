...
def get_initial_state():
    return {
        ...
        'enc_time_str': '00:00',
        'is_decryption_successful': False, # Yeni başarı durumu
    }
...
def reset_all_inputs():
    ...
    st.session_state['is_decryption_successful'] = False # Başarı durumunu da sıfırla
    
    # KALICI TUTULAN GİRDİLERİ TEMİZLE
    st.session_state['enc_secret_text_input'] = ''
    st.session_state['enc_no_pass_checkbox'] = False
    st.session_state['enc_time_str'] = '00:00'

    # KRİTİK ADIM: Dosya yükleyicilerini sıfırlamak için sayacı artır.
...
def draw_led_display(img: Image.Image, hidden_message: str) -> Image.Image:
    ...
    Bu fonksiyon, görselin üst orta kısmına şeffaf, koyu bir panel ekleyerek
    mesajı "açık" ledler şeklinde yansıtır.
    """
    ...
    # Görselin üst %30'luk kısmını kaplayacak bir alan seçimi
    ...
    # Koyu gri, yarı şeffaf arka plan (RGB=20, Opaklık=200)
    panel_draw.rectangle(panel_rect, fill=(20, 20, 20, 200))
    
    # --- 2. Mesajı Pikselleştirme ve Çizme ---

    # Ayarlar
    text_to_display = hidden_message.upper() # LED tabelalar genelde büyük harf kullanır
    led_color = (255, 69, 0, 255) # Parlak Turuncu/Kırmızı (LED Rengi)
    led_radius = 2 # Her bir "led" dairesinin yarıçapı
    dot_spacing = 8 # İki merkez nokta arasındaki mesafe
    
    # Basit bir font kullanarak mesajı çizmek
    try:
    ...
    # Geçici bir ImageDraw nesnesi üzerinde metin boyutunu hesapla
    temp_draw = ImageDraw.Draw(Image.new('RGB', (1, 1)))
    try:
        bbox = temp_draw.textbbox((0, 0), text_to_display, font=font, anchor="ls")
    ...
    # Metni panele yaz
    # Metin, LED efekti oluşturmak için geçici olarak beyaz renkte yazılır.
    panel_draw.text((start_x, start_y), text_to_display, font=font, fill=(255, 255, 255, 255))
    
    # --- 3. Pikselleştirme (LED Efekti) ---

    # Sadece metnin olduğu bölgedeki parlak pikselleri LED'lere dönüştür
    panel_pixels = panel.load()
    
    # Panelin sadece metin olan kısmını tarayalım
    for y in range(panel_rect[1], panel_rect[3], dot_spacing):
        for x in range(panel_rect[0], panel_rect[2], dot_spacing):
            
            # Koordinatları sınır içinde tut
            if x >= w or y >= h:
                continue
            
            # Pikselin RGB değerine bak (Mesajın yazıldığı yer beyaz olmalı)
            r, g, b, a = panel_pixels[x, y]
            
            # Eğer piksel parlaksa (Metin rengi), buraya parlak bir LED çiz.
            # Metin rengi beyaz olduğu için (255, 255, 255) kontrolü yapıyoruz.
            if r > 200 and a > 0:
                # Orijinal panelde sadece o pikselleri LED rengine dönüştür
                panel_draw.ellipse(
                ...
                # Mesaj olmayan yerleri koyu renkte (kapalı LED) tut
                panel_draw.ellipse(
                    (x - led_radius, y - led_radius, x + led_radius, y + led_radius),
                    fill=(50, 50, 50, 50), # Koyu, hafif şeffaf kapalı LED rengi
                    outline=None
                )
                
    # --- 4. Görüntüleri Birleştirme ---
    
    # Paneldeki beyaz metin izlerini kaldırıp sadece LED'leri bırakmak için metin çizgisini silebiliriz.
    # Ancak pikselleştirme adımı bunu zaten çözmeliydi. Birleştirme ile LED efekti sağlanır.
    
    # Resim kopyası üzerine paneli bindir (LED Tabela)
    img_copy.paste(panel, (0, 0), panel)
    
    # Orijinal RGB formatına geri dön
...
with st.sidebar:
    ...
    with st.expander("Yardım (Kullanım Kılavuzu)"):
        st.markdown(
            ...
            **GÜVENLİK NOTU:** Şifreler yalnızca form gönderimi anında kullanılır ve sunucu belleğinde kalıcı olarak tutulmaz (Client-Side modeline en yakın yaklaşım).
            """
        )
...
with tab_decrypt:
    ...
    with col1:
        ...
        with col_dec_btn:
            if st.button("🔓 Çöz", use_container_width=True): 
                ...
                # None yerine False ataması yapılmalıydı, düzeltildi.
                for k in ['decrypted_image', 'watermarked_image', 'is_message_visible', 'prompt_secret_key']:
                    st.session_state[k] = False 
                st.session_state.hidden_message = ""
                st.session_state.secret_key_hash = ""
                st.session_state.is_decryption_successful = False # Yeni: Çözme başarısını sıfırla
                            ...
                            if not allow_no and not dec_pass:
                                ...
                                if dec_img is None:
                                    ...
                                    if calc_tag != stored_tag:
                                        ...
                                        st.session_state.is_decryption_successful = True # Başarıyı işaretle
    ...
    with col2:
        ...
        # is_decryption_successful kontrolü eklendi.
        if st.session_state.is_decryption_successful and st.session_state.hidden_message:
            ...
            # Durum: Mesaj GİZLİ
                ...
                if st.button("Gizli Mesajı Göster", use_container_width=True): 
                    if st.session_state.secret_key_hash:
                        ...
                        # Filigranı ekle ve mesajı görünür yap
            ...
            if st.session_state.prompt_secret_key:
                ...
                with st.form("secret_key_form"):
                    input_key = st.text_input("Gizli Mesaj Şifresi (Filigran)", type="password")
                    if st.form_submit_button("Filigranı Aç"):
                        if hashlib.sha256(input_key.encode('utf-8')).hexdigest() == st.session_state.secret_key_hash:
                            ...
                            log("Filigran şifresi yanlış.")
                            # Şifre yanlışsa tekrar denemek için prompt_secret_key True kalır.
                            st.session_state.prompt_secret_key = True 
                            st.rerun()
